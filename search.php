...
<?php
	...
	$search=$_GET['search'];
	$db=new DB();
	$q="SELECT * FROM content_table WHERE category LIKE '%".$search."%' OR title LIKE '%".$search."%' OR author LIKE '%".$search."%' OR content LIKE '%".$search."%'"; 
	$s=$db->query($q);
	define('PAGE',3,0);
    $totalrow=$s->rowCount();
    if($totalrow>0){
		...
		$start=($page-1)*PAGE;
		$q="SELECT * FROM content_table WHERE category LIKE '%".$search."%' OR title LIKE '%".$search."%' OR author LIKE '%".$search."%' OR content LIKE '%".$search."%' ORDER BY id DESC LIMIT ".$start.",".PAGE;
		$s=$db->query($q);
		$d=new date;
		$data=new data_short;
		$limit=200;
